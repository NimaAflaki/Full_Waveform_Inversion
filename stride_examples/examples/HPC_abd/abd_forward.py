...
async def main(runtime):
    ...
    # Create problem
    name = sys.argv[1]
    num_of_trans, degrees, x = name.split('_')
    ...
    # Check if all transducers are within the computational domain

    for idx, (x, y) in enumerate(shifted_coords):
        if not (x > 0.419, y > 0.314):
            raise ValueError(f"Transducer {idx} at ({x:.6f}, {y:.6f}) is outside the computational domain:\n")
